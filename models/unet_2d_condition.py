...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        # TODO 增加了 self-attention 和 cross-attention 的 mask 以供使用到 transformer_2d函数中
        # TODO 此处的self_attention_mask和cross_attention_mask是dict，而不是一个确定的tensor
        ...
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples = downsample_block(
                    ...
                    # TODO 增加 attention mask
                    self_attention_mask=self_attention_mask if 'down' in position_to_use_mask else None,
                    cross_attention_mask=cross_attention_mask if 'down' in position_to_use_mask else None,
        ...
        if self.mid_block is not None:
            sample = self.mid_block(
                ...
                cross_attention_kwargs=cross_attention_kwargs,
                # TODO 增加 attention mask
                self_attention_mask=self_attention_mask if 'mid' in position_to_use_mask else None,
                cross_attention_mask=cross_attention_mask if 'mid' in position_to_use_mask else None,
                # use_guided_latent=use_guided_latent,
            )
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                sample = upsample_block(
                    ...
                    attention_mask=attention_mask,
                    # TODO 增加 attention mask
                    self_attention_mask=self_attention_mask if 'up' in position_to_use_mask else None,
                    cross_attention_mask=cross_attention_mask if 'up' in position_to_use_mask else None,
                    # use_guided_latent=use_guided_latent,
                )
            else:
                sample = upsample_block(
