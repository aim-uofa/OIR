...
class Attention(nn.Module):
    ...
    # def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, use_guided_latent=False, **cross_attention_kwargs):
...
class AttnProcessor:
    def __call__(
        ...
        # if use_guided_latent and attention_mask is not None:
        #     # 处理self attention mask, 因为在prepare部分会在0的位置增加head，因此需要先处理
        #     if attention_mask.shape[1] == attention_mask.shape[2]:
        #         attention_mask[0, :, :], attention_mask[attention_mask.shape[0] // 2, :, :] = 0, 0
        ...
        # if use_guided_latent and attention_mask is not None:
        #     # 处理cross attention mask，没有增加head，因此后处理
        #     if attention_mask.shape[1] != attention_mask.shape[2]:
        #         attention_mask[0, :, :], attention_mask[attention_mask.shape[0] // 2, :, :] = 0, 0
