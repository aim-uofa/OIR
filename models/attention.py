...
class AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self._use_memory_efficient_attention_xformers:
            ...
            # self_attention_mask = torch.ones(1, 4096, 4096, dtype=torch.bool).to(query_proj.device)
            # attn_mask = torch.ones(L, S, dtype=torch.bool).tril(diagonal=0) if is_causal else attn_mask
            # attn_mask = attn_mask.masked_fill(not attn_mask, -float('inf')) if attn_mask.dtype==torch.bool else attn_mask
            hidden_states = F.scaled_dot_product_attention(
                query_proj, key_proj, value_proj, dropout_p=0.0, is_causal=False,
                attn_mask=self_attention_mask[query_proj.shape[2]] if self_attention_mask is not None else self_attention_mask,
            )

            # Efficient implementation equivalent to the following:
            # attn_mask = self_attention_mask[query_proj.shape[2]]
            # attn_mask = attn_mask.masked_fill(not attn_mask, -float('inf')) if attn_mask.dtype==torch.bool else attn_mask
            # attn_weight = torch.softmax((query_proj @ key_proj.transpose(-2, -1) / math.sqrt(query_proj.size(-1))) + attn_mask, dim=-1)
            # attn_weight = torch.dropout(attn_weight, 0.0)
            # hidden_states = attn_weight @ value_proj
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # use_guided_latent=False, # 使用引导向量则引导prompt的mask就应该为None
    ):
        ...
        if self_attention_mask is None or self_attention_mask[norm_hidden_states.shape[1]] is None:
            ...
            temporal_self_attention_mask = self_attention_mask[norm_hidden_states.shape[1]].repeat(norm_hidden_states.shape[0], 1, 1)
            attn_output = self.attn1(
                norm_hidden_states,
                encoder_hidden_states=encoder_hidden_states if self.only_cross_attention else None,
                attention_mask=temporal_self_attention_mask,
                # use_guided_latent=use_guided_latent,
                **cross_attention_kwargs,
            )
        ...
        if self.attn2 is not None:
            ...
            if cross_attention_mask is None or cross_attention_mask[norm_hidden_states.shape[1]] is None:
                ...
                temporal_cross_attention_mask = cross_attention_mask[norm_hidden_states.shape[1]]
                attn_output = self.attn2(
                    norm_hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    attention_mask=temporal_cross_attention_mask,
                    # use_guided_latent=use_guided_latent,
                    **cross_attention_kwargs,
                )
            hidden_states = attn_output + hidden_states
